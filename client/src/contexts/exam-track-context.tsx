...
import { getAllFSDomains, getAllPSDomains, getAllTXDomains } from "@shared/domains";
...
export function ExamTrackProvider({ children, examTrackOverride }: ExamTrackProviderProps) {
  ...
  const value: ExamTrackContextValue = {
    ...
    isLoading,
    domains,
    examName: examTrack === 'ps' ? 'PS Exam' : examTrack === 'tx' ? 'Texas (State-Specific)' : 'FS Exam',
    lessonCount: examTrack === 'ps' ? 55 : examTrack === 'tx' ? 0 : 73,
    domainCount: examTrack === 'ps' ? 5 : examTrack === 'tx' ? 6 : 8,
  };
