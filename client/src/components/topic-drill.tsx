...
import type { DrillTopic } from '@/lib/drillTopics';
...
export function TopicDrill({ config }: { config: DrillTopic }) {
  ...
  // Topic tags only exist in the FS question pool, so filtering by `topic`
  // alone is track-safe and captures every tagged question even when a topic
  // spans more than one domain (e.g. Photogrammetry).
  const topicQuestions = useMemo(
    () => QUIZ_QUESTIONS.filter(q => q.topic === config.topic),
    [config.topic]
  );
  ...
  const handleStartDrill = () => {
    const selected = shuffleArray(topicQuestions);
    const questionsWithIds = selected.map(q => ({
      ...q,
      id: `${config.id}-${QUIZ_QUESTIONS.indexOf(q)}`
    }));
  ...
  const handleSubmit = () => {
    ...
    saveResultMutation.mutate({
      questionId: `${config.id}-drill-q-${currentQuestionIndex}-${Date.now()}`,
    ...
    return (
      <div className="p-8 max-w-4xl mx-auto">
        <div className="flex items-center gap-3 mb-2">
          ...
          <h1 className="text-3xl font-bold text-foreground" data-testid="heading-topic-drill">
            {config.title}
          </h1>
        </div>
        ...
        <Card className="p-6">
          <div className="text-center space-y-4">
            ...
            <p className="text-sm text-muted-foreground max-w-md mx-auto">
              {topicQuestions.length} {config.badgeLabel} computation question{topicQuestions.length !== 1 ? 's' : ''}.
  ...
  if (drillState === 'active' && currentQuestion) {
    ...
    const correctIndex = shuffledData?.correctIndex ?? currentQuestion.correctAnswer;
    getDomainConfig(currentQuestion.domain);
      ...
      <div className="p-8 max-w-4xl mx-auto">
        <div className="flex items-center justify-between gap-4 flex-wrap mb-6">
          <div className="flex items-center gap-3">
            ...
            <h1 className="text-xl font-bold text-foreground" data-testid="heading-topic-active">{config.title}</h1>
          </div>
          <div className="flex items-center gap-3">
            <Badge variant="outline" className="border-primary/40 text-primary">{config.badgeLabel}</Badge>
        ...
        <Progress
          ...
          className="mb-6 h-2"
          data-testid="progress-topic-drill"
        />
  ...
  if (drillState === 'complete') {
    ...
    const skillBreakdown = (() => {
      ...
      drillQuestions.forEach((q, idx) => {
        const skill = q.skill || config.skillFallback;
    ...
    return (
      <div className="p-8 max-w-4xl mx-auto">
        <h1 className="text-3xl font-bold text-foreground mb-6" data-testid="heading-topic-results">
          {config.badgeLabel} Drill Results
        </h1>
            ...
            <div className="text-center space-y-4">
              ...
              {drillAccuracy >= 80 ? (
                <Badge variant="secondary" className="text-green-600 dark:text-green-400">
                  {config.badgeLabel} computations mastered!
                </Badge>
              ) : drillAccuracy >= 60 ? (
                ...
                <Badge variant="secondary" className="text-red-600 dark:text-red-400">
                  Keep drilling {config.keepDrillingHint}
                </Badge>
              )}
            </div>
          </CardContent>
        ...
        <Card className="mb-6">
          ...
          <CardContent>
            <p className="text-sm text-muted-foreground mb-3">
              How you performed on each {config.badgeLabel} computation step.
            </p>
