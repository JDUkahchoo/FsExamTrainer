...
-   Exam Track Data Isolation: `weekProgress` and `quizSessions` tables have `examTrack` column (default 'fs') for proper data separation. Progress/overall API filters by examTrack. Client passes examTrack to all progress queries. Foundation for future Stripe-based guide monetization with isolated progress tracking per exam.
