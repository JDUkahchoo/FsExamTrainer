...
## System Architecture
The application features a responsive UI/UX with domain-specific color coding and dark mode support. **Visual design language ("Field Book to Point Cloud"):** content surfaces use an aged field-book paper aesthetic (warm cream background, ink-navy text) while the navigation sidebar is a dark "instrument panel" (sidebar tokens stay dark even in light mode). Primary is instrument teal, with brass and survey-orange accents; domain colors are preserved as functional coding. Headings use a serif (Source Serif 4) via `font-serif`/`font-display`, body uses Inter, and JetBrains Mono is used for coordinate/formula readouts. Reusable texture utilities (`bg-fieldbook`, `bg-pointcloud`, `label-coord`, `rule-brass`) live in `index.css`. Note: inside the dark sidebar use `text-sidebar-foreground`, never `text-muted-foreground`; pair `bg-survey` CTAs with dark text.

**Navigation (exam-layout.tsx):** The sidebar uses 5 grouped sections — a top-level Dashboard item, then Learn (Study Plan, Lessons, Interactive Readings), Practice (Quiz, Weak Area Drill, Flashcards, Practice Exam), Reference (Resources FS-only, Reference Companion, Procedures & Standards, Formula Reference), Track (Progress, Study Notes) — plus Settings and Change Exam in the footer. This is cleaner than the previous flat 13-item list.

**Dashboard (exam-dashboard.tsx):** 3 tabs — Overview (stats + adaptive Next Step card + quick actions), Daily Coaching (AI coach briefing, daily quests, review alerts, plan health, leaderboard, forgetting curve), Study Strategy (quick start steps, study mode selector, tips). The "Next Step" card on Overview adapts based on pretest status, lesson progress, and overall completion to guide the user to the most important action.

**Key Features:**
-   **Study Plans:** Adaptive plans (16-week for FS, 12-week for PS) with progress tracking, flexible modes, and textbook recommendations. An Adaptive Plan Generator dynamically adjusts to the user's exam date and pretest scores, cycling through distinct study phases. Includes a Week Memory Health System based on the Ebbinghaus forgetting curve, tracking study week freshness and prompting reviews via mini-quizzes. Each expanded week card is a one-stop-shop with inline domain-matched flashcards (`FlashcardWeekPreview`): shows 3 flippable preview cards + "Study All N Cards →" deep-link to the flashcard page pre-filtered for that week's domains. A **24-Month Long-Term Study Pathway** mode (FS only) spans 96 weeks across 4 phases (Foundation → Deep Dive → Integration → Exam Sprint) with pretest-aware domain allocation, phase separator headers, domain completion checkpoints, and practice exam milestone markers at phase boundaries.
...
-   **Personalized Learning:** Custom Plan Builder, Study Cycles System, Adaptive Difficulty based on performance, and Optimal Review Timing using the SM-2 algorithm for spaced repetition. All plan modes (Standard, Working Professional, Result-Driven) are now pretest-aware: when pretest results exist, domain ordering and weighting adapt to prioritize weak areas. Without pretest data, plans fall back to default domain sequences. An "Adapted from your pretest" badge appears on the study plan header when personalization is active.
