...
-   Exam Track Data Isolation: `weekProgress` and `quizSessions` tables have `examTrack` column (default 'fs') for proper data separation. All progress APIs filter by examTrack: `/api/progress/weeks`, `/api/progress/weeks/:week`, `/api/progress/overall`, `/api/progress/stats`, `/api/quiz/sessions`, `/api/quiz/stats`, `/api/exams`. Practice exams filtered by checking domainScores keys against FS_DOMAINS/PS_DOMAINS. Reading progress inherently scoped by readingId (FS/PS readings have distinct IDs). Client passes examTrack in all progress query keys and API calls. Cache invalidation uses examTrack-scoped query keys. Foundation for future Stripe-based guide monetization with isolated progress tracking per exam.
