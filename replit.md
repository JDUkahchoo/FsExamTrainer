...
-   PS Exam Track Content Scoping: Sidebar navigation hides FS-only items (Resources) for PS track via `tracks` property on menu items. Dashboard and Progress page conditionally hide FS-only resource links. All study readings are FS-only; PS study plan READ section shows chapter references without interactive readings links. Procedures, lessons, quizzes, flashcards, and practice exams properly filter by examTrack.
