...
-   Flashcard Challenge Mode: Term-matching game mode where definitions are shown and users select the correct term from a bank. Batches of 8 cards per round, domain-filtered. Split layout: definition (left), Term Bank (top-right), Matched terms (bottom-right). Tracks accuracy, wrong attempts, awards XP on completion. Component: `client/src/components/flashcard-challenge-mode.tsx`.
