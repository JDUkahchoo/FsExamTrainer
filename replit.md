...
-   **Getting Started Page (Simplified):** Entry point showing only the exam path selector (ExamSelector component). Users choose their exam track (FS or PS), then navigate to the exam-specific dashboard.
-   **ExamDashboard (Tabbed Interface):** Comprehensive study hub with 4 tabs:
    -   **Overview:** Stats cards (XP, lessons, quizzes), quick action buttons, progress summary
    -   **AI Coaching:** Study Coach Briefing, Daily Quests, Review Alerts, Weekly Leaderboard, Forgetting Curve visualization
    -   **Study Tools:** 7 study tool cards with descriptions and navigation (Study Plan, Interactive Lessons, Practice Quiz, Flashcards, Practice Exam, Study Notes, Resources)
    -   **Planner:** Quick Start steps, Study Mode selection, Tips, Ready to Start CTA
-   **Feedback, Testimonials, Privacy Policy, and Disclaimer Pages:** Additional informational pages.
...
-   **Multi-Exam Architecture with Isolated Routes:** Each exam path (FS, PS) operates as a completely isolated experience with dedicated routes, sidebar, and dashboard. Route pattern: `/app/:examTrack/*` where examTrack is 'fs' or 'ps'. ExamLayout component wraps all exam-specific pages with SidebarProvider, exam-specific navigation, and "Change Exam" footer button. ExamRoutes component handles routing under `/app/:examTrack/` paths including dashboard, study-plan, lessons, flashcards, quizzes, practice-exam, progress, notes, and reference pages. ExamTrackProvider derives exam track from URL params (falling back to user preferences). This architecture supports future payment/monetization where users purchase access to individual exam tracks. `EXAM_TRACKS` constant defines available exam tracks with status flags. `US_STATES` constant lists all 50 states with availability flags. Backend validates exam track selection (fs and ps allowed, state-specific blocked). Lessons, flashcards, and queries are filtered by `examTrack` field. `NCEES_PS_DOMAINS` in `shared/domains.ts` defines PS exam structure. PS lesson IDs use `ps-d{domain}-lesson-{orderIndex:02}` format.
-   **ExamTrackProvider Context:** Client-side React context (`client/src/contexts/exam-track-context.tsx`) centralizes exam track state and provides exam-specific configuration across the app. Exports `examTrack`, `domains` (array with number and name), `examName`, `lessonCount`, and `domainCount`. Domain configuration (`client/src/lib/domains.ts`) extended with PS domain styling (maps PS domains to appropriate colors/icons). Pages conditionally render "Coming Soon" placeholders for PS features not yet implemented (Practice Quiz, Practice Exam, Study Plan). Flashcards page filters decks by exam track (PS users only see original deck with PS cards).
-   **Future Enhancements:** Domain-based study plan UI for PS users (currently shows "Coming Soon"). PS-specific practice quizzes and exams. Onboarding flow should require explicit exam track selection before loading study data.
