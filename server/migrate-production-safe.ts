...
// SAFE Production migration - checks for existing data first
// Run with: NODE_ENV=production tsx server/migrate-production-safe.ts
...
async function safeProductionMigration() {
  ...
  if (process.env.NODE_ENV !== 'production') {
    ...
    console.log("Run with: NODE_ENV=production tsx server/migrate-production-safe.ts");
  }

  try {
    // Step 1: COMPREHENSIVE safety check of ALL user data tables
    console.log("🔍 Checking ALL user data tables for safety...\n");
    ...
    // Step 2: Safety gate - STOP if ANY user data exists
    if (hasAnyUserData) {
      console.error("\n❌ SAFETY STOP: User data detected!");
      console.error("   Cannot proceed - would orphan existing user data");
      console.error("\n🛑 Action Required:");
      console.error("   Contact developer for UPDATE-based migration strategy");
      process.exit(1);
    }
    
    // Step 3: If completely empty, safe to proceed
    console.log("\n✅ Safety check passed - no user data found");
    console.log("🚀 Starting migration with transaction...\n");
    
    await db.transaction(async (tx) => {
      // Delete in correct order (questions first, then lessons)
      ...
      // Insert lessons and questions within transaction
