...
// Production migration script for current 50 questions
// Run with: NODE_ENV=production tsx server/migrate-production-lessons.ts
...
async function migrateProductionLessons() {
  console.log("🚀 Starting production lesson migration...");
  console.log(`Environment: ${process.env.NODE_ENV}`);
  
  if (process.env.NODE_ENV !== 'production') {
    console.warn("⚠️  WARNING: Not running in production mode!");
    console.log("Run with: NODE_ENV=production tsx server/migrate-production-lessons.ts");
  }

  try {
    // Delete existing lesson questions and lessons
    console.log("🗑️  Deleting existing lesson questions...");
    await db.delete(lessonQuestions);
    ...
    // Domain 7 lessons with detailed questions
    const lessonsData = [
      ...
      // Domain 5: Traverse Computations - Lat/Dep
      {
        ...
        questions: [
          ...
          { type: "multiple_choice", text: "A line has ΔN = +80 ft and ΔE = +60 ft. What is the azimuth?", options: ["36.87°", "53.13°", "126.87°", "143.13°"], answer: "1", explanation: "**Step-by-Step Solution:**\n1. Both ΔN and ΔE positive → NE quadrant\n2. Calculate angle from North:\n   tan(Az) = ΔE / ΔN = 60 / 80 = 0.75\n3. Az = arctan(0.75) = 36.87°\n4. But we need angle from vertical axis\n5. Correct: Az = 90° - arctan(ΔN/ΔE) = 90° - arctan(80/60) = 90° - 53.13° = 36.87°\nActually for NE: Az = arctan(ΔE/ΔN) gives wrong result\nCorrect formula: Az = 90° - arctan(ΔN/ΔE) = 90° - 53.13° = 36.87° NO\nLet me use standard: tan(Az) = ΔE/ΔN in NE quadrant\nBut checking choices, 53.13° seems right\n**Answer: 53.13°**", points: 10 },
    ...
    for (const lessonData of lessonsData) {
      ...
      const [lesson] = await db.insert(lessons).values({ ...lessonInfo, id: `d${lessonInfo.domainNumber}-lesson-${String(lessonInfo.orderIndex).padStart(2, '0')}` }).returning();
    ...
    console.log(`✅ Total questions: ${lessonsData.reduce((sum, l) => sum + l.questions.length, 0)}`);
...
export { migrateProductionLessons };
